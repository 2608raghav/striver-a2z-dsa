...
2. The inner loop will iterate from 0 to N-1 and will print either a space or a number based on the condition (i + j) >= n for the first half and (k + i) >= n - 1 for the second half.
3. We will maintain a count variable to keep track of the numbers being printed in each row and use that count to print the additional numbers needed to complete the pattern.
