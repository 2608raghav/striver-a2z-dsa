...
5. Repeat steps 2-4 until the start pointer is equal to the end pointer. The element at the start (or end) pointer will be the minimum element in the array.


Time Complexity: O(log n) in the worst and average cases, as we are dividing the search space in half at each step. In the best case (when the array is already sorted), the time complexity is O(1) as we can directly return the first element.
