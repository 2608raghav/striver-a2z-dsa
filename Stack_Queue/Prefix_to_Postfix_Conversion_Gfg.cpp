...
3. After processing all characters, the stack will contain one element, which is the fully formed postfix expression.
    ...
    string prefixToPostfix(string s) {
        ...
        for (int i = s.length() - 1; i >= 0; i--) {
            ...
            if (isalnum(c)) {
                ...
                string temp = op1 + op2 + c; // Combine with operator in postfix order
                st.push(temp); // Push the resulting postfix expression back to stack
            }
        }
        
        return st.top(); // The final postfix expression
    }
};